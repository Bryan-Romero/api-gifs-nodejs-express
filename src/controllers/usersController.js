    ...
    conn.query(
      ...
      (err, users) => {
        ...
        if (userFound === 0) {
          return res
            .status(400)
            .json({ message: "Error in email or password" });
        } else {
          //validate password
          req.getConnection((err, conn) => {
            if (err) return res.json(err);
            conn.query(
              ...
              (err, userP) => {
                if (err) return res.json(err);
                else if (password !== userP[0].password)
                  return res
                    .status(400)
                    .json({ message: "Error in email or password" });
                ...
                req.getConnection((err, conn) => {
                  if (err) return res.json(err);
                  conn.query(
                    "SELECT idUser, name, lastName FROM users where email = ?",
                    [email],
                    (err, user) => {
                      if (err) return res.json(err);
    ...
    if (error) {
      ...
      foundIsFav(req, idGif, idUser).then((resolve) => {
        ...
        req.getConnection((err, conn) => {
          if (err) return res.json(err);
          conn.query(
            "INSERT INTO favorites (FK_idUser, idGif) VALUES (?, ?)",
            [idUser, idGif],
            (err, rows) => {
              if (err) return res.json(err);
              ...
              req.getConnection((err, conn) => {
                if (err) return res.json(err);
                conn.query(
                  "SELECT idGif FROM favorites where FK_idUser = ?",
                  [idUser],
                  (err, rows) => {
                    if (err) return res.json(err);
  ...
  jwt.verify(req.token, "secretkey", (error, authData) => {
    if (error) {
      ...
      req.getConnection((err, conn) => {
        if (err) return res.json(err);
        conn.query(
          "DELETE FROM favorites WHERE (FK_idUser = ? and idGif = ?)",
          [idUser, idGif],
          (err, rows) => {
            if (err) return res.json(err);
...
const foundIsFav = (req, idGif, idUser) => {
  return new Promise((resolve) => {
    req.getConnection((err, conn) => {
      ...
      conn.query(
        "SELECT idfavorites FROM favorites where (FK_idUser = ? and idGif = ?)",
        [idUser, idGif],
        (err, rows) => {
          if (err) return err;
